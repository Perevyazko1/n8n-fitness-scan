...
> **Сессия 2026-06-04:** написаны Фазы 0 и 1 (фронт + дашборд-вебхук). Код локально
> готов, НЕ закоммичен/не задеплоен — это за пользователем. Дальше после деплоя:
> проверить дашборд в Telegram, затем Фаза 2 (логи еды).
...
### Фаза 1 — Главная (дашборд) — `[~]` код готов, ждёт деплоя
...
### Фаза 3 — Упражнения с тогглами
- [ ] Данные: завести лист `workout_done` (схема выше).
- [ ] Бэк: `POST /webhook/workout-today` (мерж плана из `workouts_flat` + галочки).
- [ ] Бэк: `POST /webhook/toggle-exercise` (upsert в `workout_done`).
- [ ] Фронт: список упражнений со свитчерами; оптимистичный тоггл + POST + откат при
      ошибке.
- РЕШЕНИЕ: тогглы **персистентные** (держатся между открытиями). Если передумаем на
  «в рамках сессии» — Фаза 3 сильно упрощается (без `workout_done` и вебхуков записи).
